...
public class ProcPlaneEditor : Editor
{
    protected virtual void OnSceneGUI()
    {
        ...
        Vector3 newOffset = Handles.DoPositionHandle(procTarget.transform.position + procTarget.CellOffset, 
                                                     procTarget.transform.rotation);

        if (EditorGUI.EndChangeCheck() )
        {
            Undo.RecordObject(procTarget, "Procedural Values Changed!");
            procTarget.CellOffset = newOffset - procTarget.transform.position;
